-- =============================================================================
-- MAMMA MIA — Stats : total par personne qui a PRIS la commande
-- Compte order_items.created_by (envoi), jamais orders.paid_by (caisse).
-- A executer dans Supabase > SQL Editor, APRES 0019_stats_zones.sql.
-- Rejouable. Lecture seule.
-- =============================================================================
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(r) order by r.total_cents desc, r.name), '[]'::jsonb)
    ...
    from (
      select
        coalesce(s.full_name, '__guest__') as name,
        coalesce(sum(oi.line_total_cents), 0)::integer as total_cents,
        coalesce(sum(oi.quantity), 0)::integer as item_count
      from public.order_items oi
      join public.orders o on o.id = oi.order_id
      left join public.staff s on s.id = oi.created_by
      where o.status <> 'cancelled'
        and coalesce(oi.sent_at, oi.created_at) >= v_start
        and coalesce(oi.sent_at, oi.created_at) < v_end
        and coalesce(oi.status::text, 'sent') = 'sent'
      group by coalesce(s.full_name, '__guest__')
    ) r;
