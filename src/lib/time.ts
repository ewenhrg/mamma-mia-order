...
export function formatElapsed(from: string | number | Date, now = Date.now()): string {
  ...
  if (minutes < 1) return translate(locale, 'time.lt1');
  if (minutes < 60) return translate(locale, 'time.min', { n: minutes });

  const hours = Math.floor(minutes / 60);
  const rest = minutes % 60;
  return rest === 0
    ? translate(locale, 'time.h', { n: hours })
    : translate(locale, 'time.hmin', { h: hours, m: String(rest).padStart(2, '0') });
}
